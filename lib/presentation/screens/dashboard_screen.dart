...
class _DashboardScreenState extends ConsumerState<DashboardScreen>
                  ...
                  data: (reports) {
                    ...
                    return Column(
                      children: recentReports
                          .map((report) => _buildReportTileFromFirebase(report))
                          .toList(),
                    );
                  },
                  loading: () => Column(
                    children:
                        List.generate(3, (index) => _buildLoadingReportTile()),
                  ),
  ...
  Widget _buildReportTileFromFirebase(Map<String, dynamic> report) {
  ...
  Widget _buildLoadingReportTile() {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildRiskOverviewSection() {
    return SliverToBoxAdapter(
      child: Container(
        margin: const EdgeInsets.all(16),
        padding: const EdgeInsets.all(20),
        ...
        child: Column(
          ...
          children: [
            ...
            Consumer(
              builder: (context, ref, child) {
                final highRiskAsync = ref.watch(highRiskVillagesProvider);
                  ...
                  data: (villages) {
                    ...
                    return SizedBox(
                      ...
                      child: ListView(
                        scrollDirection: Axis.horizontal,
                        children: villages
                            .map((village) => _buildVillageRiskChip(
                                  village['name'],
                                  _getRiskColor(village['riskLevel']),
                                ))
                            .toList(),
                      ),
                    );
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed: () async {
                  // Add sample data to test Firebase connection
                  try {
                    await FirebaseFirestore.instance.collection('reports').add({
                      'intent': 'SYMPTOM',
                      'phone': '919876543210',
                      'message':
                          'Test report from app - Multiple people experiencing fever and diarrhea.',
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Sample report added successfully!'),
                ...
                icon:
                    const Icon(Icons.add_circle_outline, color: AppColors.safe),
                label: const Text(
  ...
  Widget _buildVillageRiskChip(String name, Color riskColor) {
    return Container(
      margin: const EdgeInsets.only(right: 8),
